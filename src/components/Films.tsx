...
const FilmsFragment = graphql`
  ...
  @argumentDefinitions(first: { type: "Int", defaultValue: 10 }, after: { type: "String" }) {
    allFilms(first: $first, after: $after) @connection(key: "FilmsFragment_allFilms") {
      edges {
        node {
          id
          ...FilmCardFragment
...
export const Films = ({ query: queryFragment }: Props) => {
  ...
  return (
    <>
      ...
      <h1>All Films</h1>
      <Cards>{allFilms?.edges?.map((edge) => edge?.node && <FilmCard key={edge.node.id} film={edge.node} />)}</Cards>
